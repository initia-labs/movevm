...
macro_rules! safely_pop_arg {
    ($args:ident, $t:ty) => {{
        use $crate::interface::reexports::move_vm_types::natives::function::{
            PartialVMError, StatusCode,
        };
        match $args.pop_back() {
            Some(val) => match val.value_as::<$t>() {
                Ok(v) => v,
                Err(e) => return Err($crate::interface::SafeNativeError::InvariantViolation(e)),
            },
            None => {
                return Err($crate::interface::SafeNativeError::InvariantViolation(
...
macro_rules! safely_get_struct_field_as {
    ($value:expr, $idx:expr, $t:ty) => {{
        ...
        $value
            .borrow_field($idx)
            .map_err($crate::interface::SafeNativeError::InvariantViolation)?
            .value_as::<Reference>()
            .map_err($crate::interface::SafeNativeError::InvariantViolation)?
            .read_ref()
            .map_err($crate::interface::SafeNativeError::InvariantViolation)?
...
macro_rules! safely_pop_vec_arg {
    ($arguments:ident, $t:ty) => {{
        ...
        use $crate::interface::reexports::move_vm_types::natives::function::{PartialVMError, StatusCode};
        let value_vec = match $arguments.pop_back().map(|v| v.value_as::<Vec<Value>>()) {
            ...
            Some(Err(e)) => return Err($crate::interface::SafeNativeError::InvariantViolation(e)),
            Some(Ok(v)) => v,
        };
