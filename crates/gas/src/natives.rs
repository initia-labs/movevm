...
macro_rules! define_gas_parameters_for_natives {
    ($param_ty: ty, $package_name: literal, [$([$($t: tt)*]),* $(,)?] $(, allow_unmapped = $allow_unmapped: expr)?) => {
        impl crate::meter::FromOnChainGasSchedule for $param_ty {
        ...
        impl crate::meter::ToOnChainGasSchedule for $param_ty {
        ...
        impl crate::meter::InitialGasSchedule for $param_ty {
            fn initial() -> Self {
                let mut params = <$param_ty>::zeros();
