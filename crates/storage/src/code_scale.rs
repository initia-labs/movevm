...
use move_vm_types::code::WithBytes;
...
impl WeightScale<Checksum, Code<CompiledScript, Script>> for CodeScale {
    fn weight(&self, _key: &Checksum, value: &Code<CompiledScript, Script>) -> usize {
        unsafe{
            let value = &*(value as *const Code<CompiledScript, Script> as *const MyCode<MyCompiledScript, MyScript>);
            value.get_size()
        }
    }
}
...
impl WeightScale<Checksum, Arc<ModuleCode<CompiledModule, Module, BytesWithHash, NoVersion>>> for ModuleCodeScale {
    fn weight(&self, _key: &Checksum, value: &Arc<ModuleCode<CompiledModule, Module, BytesWithHash, NoVersion>>) -> usize {
        unsafe{
            let value = &*(value.as_ref() as *const ModuleCode<CompiledModule, Module, BytesWithHash, NoVersion> as *const MyModuleCode<MyCompiledModule, MyModule, BytesWithHash, NoVersion>);
            value.get_size()
        }
    }
}