...
def movevm_supported(os, cpu, build_type):
    ...
    if os == "macOS" and build_type == "shared":
        return SUPPORTED + "libinitia.dylib"
    if os == "Linux (musl)":
        if build_type == "static":
            if cpu == "x86_64":
                return SUPPORTED + "libinitia_muslc.x86_64.a"
            elif cpu == "aarch64":
                return SUPPORTED + "libinitia_muslc.aarch64.a"
    ...
    if os == "Linux (glibc)":
        ...
        if build_type == "shared":
            if cpu == "x86_64":
                return SUPPORTED + "libinitia.x86_64.so"
            elif cpu == "aarch64":
                return SUPPORTED + "libinitia.aarch64.so"
    return UNKNOWN

def get_note(os, cpu, build_type):
    if os == "Linux (glibc)" and cpu == "x86_64" and build_type == "static":
        return "Would link libinitia statically but glibc dynamically as static glibc linking is not recommended. Potentially interesting for Osmosis."
...
print("<!-- AUTO GENERATED BY libinitia_builds.py START -->")
...
# print(get_links())
print("<!-- AUTO GENERATED BY libinitia_builds.py END -->")
