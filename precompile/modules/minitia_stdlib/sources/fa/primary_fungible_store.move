...
module minitia_std::primary_fungible_store {
    ...
    public fun ensure_primary_store_exists<T: key>(owner: address, metadata: Object<T>,)
    ...
    public fun primary_store_exists<T: key>(account: address, metadata: Object<T>): bool {
        fungible_asset::store_exists(
            primary_store_address(account, metadata)
        )
    }
    ...
    public fun balances(
        ...
        while (
            table::prepare<address, address>(&mut iter) && vector::length(&balance_vec) <(
                limit as u64
            )
        ) {
    ...
    public fun mint(
        ...
        let primary_store = ensure_primary_store_exists(
            owner,
    ...
    #[test_only]
    public fun init_test_metadata_with_primary_store_enabled(constructor_ref: &ConstructorRef)
        : (
    ...
    fun test_default_behavior(
        ...
        assert!(
            balance(creator_address, metadata) == 0,
            3
        );
        assert!(
            balance(aaron_address, metadata) == 0,
            4
        );
        assert!(
            !is_frozen(creator_address, metadata),
            5
        );
        assert!(
            !is_frozen(aaron_address, metadata),
            6
        );
    ...
    fun test_basic_flow(
        ...
        assert!(
            balance(creator_address, metadata) == 30,
            3
        );
        assert!(
            balance(aaron_address, metadata) == 70,
            4
        );
