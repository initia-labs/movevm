module minitia_std::dex {
    ...
    public fun get_spot_price(
        ...
        let (coin_a_pool, coin_b_pool, coin_a_weight, coin_b_weight, _) = pool_info(pair,
            false);

        let pair_key = generate_pair_key(pair);
        let base_addr = object::object_address(base_coin);
        assert!(base_addr == pair_key.coin_a || base_addr == pair_key.coin_b,
        ...
        let (base_pool, quote_pool, base_weight, quote_weight) =
            if (is_base_a) { (coin_a_pool, coin_b_pool, coin_a_weight, coin_b_weight) }
            else { (coin_b_pool, coin_a_pool, coin_b_weight, coin_a_weight) };

        decimal128::from_ratio_u64(decimal128::mul_u64(&base_weight, quote_pool),
            decimal128::mul_u64(&quote_weight, base_pool),)
    }

    #[view]
    public fun get_spot_price_by_denom(pair_denom: String, base_coin: String,): Decimal128 acquires Config, Pool {
    ...
    public fun get_swap_simulation(
        ...
        assert!(offer_address == pair_key.coin_a || offer_address == pair_key.coin_b,
            error::invalid_argument(ECOIN_TYPE),);
        ...
        let (offer_pool, return_pool, offer_weight, return_weight) =
            if (is_offer_a) { (pool_a, pool_b, weight_a, weight_b) }
            else { (pool_b, pool_a, weight_b, weight_a) };
        let (return_amount, _fee_amount) =
            swap_simulation(offer_pool,
                return_pool,
                offer_weight,
                return_weight,
                offer_amount,
    ...
    public fun get_swap_simulation_given_out(
        ...
        let (offer_amount, _fee_amount) =
            swap_simulation_given_out(offer_pool,
                return_pool,
                offer_weight,
                return_weight,
                return_amount,
    ...
    public fun get_swap_simulation_given_out_by_denom(
        ...
        let pair_metadata = coin::denom_to_metadata(pair_denom);
        let offer_metadata = coin::denom_to_metadata(offer_denom);
        get_swap_simulation_given_out(object::convert(pair_metadata), offer_metadata,
            return_amount)
    }
    ...
    public fun get_all_pairs(
        ...
        assert!(option::is_some(&coin_a_start_after)
            == option::is_some(&coin_b_start_after) && option::is_some(&coin_b_start_after)
        ...
        let start_after =
            if (option::is_some(&coin_a_start_after)) {
                ...
                option::some(PairKey { coin_a: @0x0, coin_b: @0x0, liquidity_token: @0x0, })
            };

        let res = vector[];
        let pairs_iter =
            table::iter(&module_store.pairs, start_after, option::none(), 1,);

        while (vector::length(&res) < (limit as u64) && table::prepare<PairKey, PairResponse>(
                pairs_iter)) {
        ...
        let start_after =
            if (option::is_some(&coin_a_start_after)) {
                ...
                let liquidity_token_start_after =
                    coin::denom_to_metadata(option::extract(&mut liquidity_token_start_after));
                option::some(PairKey {
            ...
            if (&key != option::borrow(&start_after)) {
                vector::push_back(&mut res,
                    PairByDenomResponse {
                        ...
                        liquidity_token: coin::metadata_to_denom(object::address_to_object(value
                                    .liquidity_token)),
    ...
    public fun get_pairs(
        coin_a: address, coin_b: address, start_after: Option<address>, limit: u8,
    ): vector<PairResponse> acquires ModuleStore {
        if (limit > MAX_LIMIT) {
            limit = MAX_LIMIT;
        };
    ...
    public fun unpack_pair_response(pair_response: &PairResponse)
        : (address, address, address, Weights,
        Decimal128) {
    ...
    fun check_chain_permission(chain: &signer) {
        assert!(signer::address_of(chain) == @minitia_std,
    ...
    public entry fun create_pair_script(
        ...
        let liquidity_token =
            create_pair(creator, name, symbol, swap_fee_rate, coin_a, coin_b, weights);
    ...
    public entry fun update_swap_fee_rate(
        ...
        assert!(decimal128::val(&swap_fee_rate) <= MAX_FEE_RATE,
        ...
        let pair_response =
            table::borrow_mut(&mut module_store.pairs, pair_key,);

        pair_response.swap_fee_rate = swap_fee_rate;

        // emit event
        event::emit<SwapFeeUpdateEvent>(SwapFeeUpdateEvent {
    ...
    public entry fun provide_liquidity_script(
        ...
        provide_liquidity_from_coin_store(account, pair, coin_a_amount_in, coin_b_amount_in,
            min_liquidity,);
    }
    ...
    public fun provide_liquidity_from_coin_store(
        ...
        let (coin_a, coin_b) =
            if (total_share == 0) {
                (coin::withdraw(account, fungible_asset::store_metadata(pool.coin_a_store),
                        coin_a_amount_in),
                    coin::withdraw(account,
                        fungible_asset::store_metadata(pool.coin_b_store), coin_b_amount_in),
                )
            } else {
                let coin_a_share_ratio = decimal128::from_ratio_u64(coin_a_amount_in,
                    coin_a_amount);
                let coin_b_share_ratio = decimal128::from_ratio_u64(coin_b_amount_in,
                    coin_b_amount);
                if (decimal128::val(&coin_a_share_ratio) > decimal128::val(&coin_b_share_ratio)) {
                    coin_a_amount_in = decimal128::mul_u64(&coin_b_share_ratio,
                        coin_a_amount);
                } else {
                    coin_b_amount_in = decimal128::mul_u64(&coin_a_share_ratio,
                        coin_b_amount);
                };
        ...
        let liquidity_token =
            provide_liquidity(pair, coin_a, coin_b, min_liquidity,);
    ...
    public entry fun withdraw_liquidity_script(
        ...
        let liquidity_token =
            coin::withdraw(account, object::convert<Config, Metadata>(pair), liquidity);
        let (coin_a, coin_b) =
            withdraw_liquidity(liquidity_token, min_coin_a_amount, min_coin_b_amount,);
    ...
    public entry fun swap_script(
        ...
        assert!(option::is_none(&min_return) || *option::borrow(&min_return) <= fungible_asset::amount(
                &return_coin),
    ...
    public fun withdraw_liquidity(
        lp_token: FungibleAsset, min_coin_a_amount: Option<u64>, min_coin_b_amount: Option<u64>,
        ...
        let total_share =
            option::extract(&mut fungible_asset::supply(fungible_asset::metadata_from_asset(
                        &lp_token)));
        let coin_a_amount = fungible_asset::balance(pool.coin_a_store);
        ...
        assert!(option::is_none(&min_coin_a_amount) || *option::borrow(&min_coin_a_amount)
            <= coin_a_amount_out,
            error::invalid_state(EMIN_WITHDRAW),);
        assert!(option::is_none(&min_coin_b_amount) || *option::borrow(&min_coin_b_amount)
            <= coin_b_amount_out,
            error::invalid_state(EMIN_WITHDRAW),);
    ...
    public fun single_asset_provide_liquidity(
        pair: Object<Config>, provide_coin: FungibleAsset, min_liquidity_amount: Option<u64>,
        ...
        let pair_key = generate_pair_key(pair);
        assert!(provide_address == pair_key.coin_a || provide_address == pair_key.coin_b,
            error::invalid_argument(ECOIN_TYPE),);
        ...
        let (normalized_weight, pool_amount_in, provide_coin_addr) =
            if (is_provide_a) {
                let normalized_weight =
                    decimal128::from_ratio(decimal128::val(&coin_a_weight),
                ...
                let normalized_weight =
                    decimal128::from_ratio(decimal128::val(&coin_b_weight),
                        decimal128::val(&coin_a_weight) + decimal128::val(&coin_b_weight));
        ...
        let adjusted_swap_amount =
            decimal128::mul_u64(&decimal128::sub(&decimal128::one(), &normalized_weight),
                amount_in);
        let fee_amount =
        ...
        let base =
            decimal128::from_ratio_u64(adjusted_amount_in + pool_amount_in, pool_amount_in);
        ...
        assert!(option::is_none(&min_liquidity_amount) || *option::borrow(&min_liquidity_amount)
            <= liquidity,
            error::invalid_state(EMIN_LIQUIDITY),);

        // emit events
        event::emit<SingleAssetProvideEvent>(SingleAssetProvideEvent {
                coin_a: pair_key.coin_a,
                coin_b: pair_key.coin_b,
    ...
    public fun swap(pair: Object<Config>, offer_coin: FungibleAsset,): FungibleAsset acquires Config, Pool {
        ...
        let (pool_a, pool_b, weight_a, weight_b, swap_fee_rate) = pool_info(pair, true);
        let (offer_coin_addr, return_coin_addr, offer_pool, return_pool, offer_weight,
            return_weight) =
            if (is_offer_a) { (pair_key.coin_a, pair_key.coin_b, pool_a, pool_b, weight_a, weight_b) }
            else { (pair_key.coin_b, pair_key.coin_a, pool_b, pool_a, weight_b, weight_a) };
    ...
    public fun create_pair(
        ...
        let (mint_cap, burn_cap, freeze_cap, extend_ref) =
            coin::initialize_and_generate_extend_ref(creator,
                option::none(),
                name,
                symbol,
                6,
        ...
        assert!(coin_address(&coin_a) != coin_address(&coin_b),
        ...
        let coin_a_store =
            primary_fungible_store::create_primary_store(pair_address,
        ...
        move_to(pair_signer, CoinCapabilities { mint_cap, freeze_cap, burn_cap },);
        ...
        let liquidity_token =
            provide_liquidity(object::address_to_object<Config>(pair_address),
                coin_a,
                coin_b,
                option::none(),);
        ...
        let pair_key =
            PairKey { coin_a: coin_a_addr, coin_b: coin_b_addr, liquidity_token: pair_address };

        // add pair to table for queries
        table::add(&mut module_store.pairs,
            pair_key,
            PairResponse {
                coin_a: coin_a_addr,
                coin_b: coin_b_addr,
        ...
        event::emit<CreatePairEvent>(CreatePairEvent {
                coin_a: coin_a_addr,
                coin_b: coin_b_addr,
                liquidity_token: pair_address,
                weights,
    ...
    public fun provide_liquidity(
        ...
        let liquidity =
            if (total_share == 0) {
                if (coin_a_amount_in > coin_b_amount_in) { coin_a_amount_in }
                else { coin_b_amount_in }
            } else {
                let coin_a_share_ratio = decimal128::from_ratio_u64(coin_a_amount_in,
                    coin_a_amount);
                let coin_b_share_ratio = decimal128::from_ratio_u64(coin_b_amount_in,
                ...
                } else { (decimal128::mul_u128(&coin_a_share_ratio, total_share) as u64) }
            };
        ...
        event::emit<ProvideEvent>(ProvideEvent {
    ...
    fun check_lbp_ended(weights: &Weights) {
        ...
        assert!(timestamp >= weights.weights_after.timestamp,
    ...
    fun get_weight(weights: &Weights): (Decimal128, Decimal128) {
        ...
        if (timestamp <= weights.weights_before.timestamp) {
            ...
            let coin_a_m =
                decimal128::new(decimal128::val(&weights.weights_after.coin_a_weight) * time_diff_before);
            let coin_a_n =
                decimal128::new(decimal128::val(&weights.weights_before.coin_a_weight) * time_diff_after);
            let coin_a_l = decimal128::add(&coin_a_m, &coin_a_n);

            let coin_b_m =
                decimal128::new(decimal128::val(&weights.weights_after.coin_b_weight) * time_diff_before);
            let coin_b_n =
                decimal128::new(decimal128::val(&weights.weights_before.coin_b_weight) * time_diff_after);
    ...
    public fun pool_info(pair: Object<Config>, lbp_assertion: bool)
        ...
        if (lbp_assertion) {
            ...
            let (_, timestamp) = get_block_info();
            assert!(timestamp >= config.weights.weights_before.timestamp,
    ...
    public fun swap_simulation(
        ...
        let exp =
            decimal128::from_ratio(decimal128::val(&weight_in), decimal128::val(&weight_out));

        let fee_amount = calculate_fee_with_minimum(&swap_fee_rate, amount_in);
        let adjusted_amount_in = amount_in - fee_amount;
        let base =
            decimal128::from_ratio_u64(pool_amount_in, pool_amount_in + adjusted_amount_in);
        let sub_amount = pow(&base, &exp);
        (decimal128::mul_u64(&decimal128::sub(&one, &sub_amount), pool_amount_out),
            fee_amount)
    }
    ...
    public fun swap_simulation_given_out(
        ...
        let one = decimal128::one();
        let exp =
            decimal128::from_ratio(decimal128::val(&weight_out), decimal128::val(&weight_in));
        let base =
            decimal128::from_ratio_u64(pool_amount_out, pool_amount_out - amount_out);
    ...
    fun pow(base: &Decimal128, exp: &Decimal128): Decimal128 {
        assert!(decimal128::val(base) != 0 && decimal128::val(base) < 2000000000000000000,
    ...
    fun ln(num: &Decimal128): (Decimal128, bool) {
        ...
        let (a, a_neg) =
            if (num_val >= one) { (decimal128::sub(num, &decimal128::one()), false) }
            else { (decimal128::sub(&decimal128::one(), num), true) };
        ...
        while (decimal128::val(&comp) > PRECISION) {
            ...
            comp = decimal128::div(&decimal128::new(decimal128::val(&mul_decimal128s(&comp, &a))
                        * index), // comp * a * index
                index + 1,);
    ...
    fun mul_decimal128s(decimal128_0: &Decimal128, decimal128_1: &Decimal128): Decimal128 {
        ...
        let val_mul =
            (decimal128::val(decimal128_0) as u256) * (decimal128::val(decimal128_1) as u256);
    ...
    fun initialized_coin(account: &signer, symbol: String,)
        ...
        let (mint_cap, burn_cap, freeze_cap, _) =
            coin::initialize_and_generate_extend_ref(account,
                option::none(),
                string::utf8(b""),
                symbol,
                6,
    ...
    fun end_to_end(chain: signer,) acquires Config, CoinCapabilities, ModuleStore, Pool {
        ...
        let (initia_burn_cap, initia_freeze_cap, initia_mint_cap) = initialized_coin(&chain,
            string::utf8(b"INIT"));
        let (usdc_burn_cap, usdc_freeze_cap, usdc_mint_cap) = initialized_coin(&chain,
            string::utf8(b"USDC"));
        ...
        assert!(coin::balance(chain_addr, init_metadata)
            == 20000000 - 1000 + 997, 5); // return 1000 commission 3
        assert!(coin::balance(chain_addr, usdc_metadata)
            == 80000000 + 996 - 1000, 6);

        // withdraw liquidity
        withdraw_liquidity_script(&chain, pair, 40000000, option::none(), option::none());
        assert!(coin::balance(chain_addr, init_metadata)
            == 20000000 - 1000 + 997 + 40000001, 7);
        assert!(coin::balance(chain_addr, usdc_metadata)
            == 80000000 + 996 - 1000 + 10000002, 8);

        // single asset provide liquidity (coin b)
        // pool balance - init: 40000002, usdc: 10000002
        single_asset_provide_liquidity_script(&chain, pair, usdc_metadata, 100000,
            option::none());
        ...
        single_asset_provide_liquidity_script(&chain, pair, init_metadata, 100000,
            option::none());
        assert!(coin::balance(chain_addr, lp_metadata)
            == 40000000 + 79491 + 80090, 10);
    ...
    fun lbp_end_to_end(chain: signer,) acquires Config, CoinCapabilities, ModuleStore, Pool {
        ...
        assert!(get_spot_price(pair, init_metadata)
            == decimal128::from_string(&string::utf8(b"24.75")), 0,);

        // 0.8 : 0.2
        set_block_info(11, 2500);
        assert!(get_spot_price(pair, init_metadata)
            == decimal128::from_string(&string::utf8(b"1")), 1,);
        ...
        swap_script(&chain, pair, usdc_metadata, 1000, option::none());
        assert!(coin::balance(chain_addr, init_metadata)
            == 20000000 - 1000 + 997, 6); // return 1000 commission 3
        assert!(coin::balance(chain_addr, usdc_metadata)
            == 80000000 + 996 - 1000, 7);
    ...
    fun get_weight_test() {
        ...
        assert!(coin_a_weight == decimal128::from_ratio(2, 10) && coin_b_weight == decimal128::from_ratio(
                8, 10),
            0,);

        set_block_info(15, 1500);
        let (coin_a_weight, coin_b_weight) = get_weight(&weights);
        assert!(coin_a_weight == decimal128::from_ratio(5, 10) && coin_b_weight == decimal128::from_ratio(
                5, 10),
            1,);

        set_block_info(20, 2000);
        let (coin_a_weight, coin_b_weight) = get_weight(&weights);
        assert!(coin_a_weight == decimal128::from_ratio(8, 10) && coin_b_weight == decimal128::from_ratio(
                2, 10),
            2,);
    ...
    fun get_pair_test(chain: signer) acquires CoinCapabilities, Config, Pool, ModuleStore {
        ...
        let weights =
            Weights {
                weights_before: Weight { coin_a_weight: weight, coin_b_weight: weight, timestamp },
                weights_after: Weight { coin_a_weight: weight, coin_b_weight: weight, timestamp }
            };

        let res =
            get_all_pairs(option::none(), option::none(), option::none(), 10);
        assert!(res
            == vector[
                PairResponse {
                    coin_a: a_addr,
        ...
        let res =
            get_all_pairs(option::some(a_addr),
                option::some(b_addr),
        ...
        let res =
            get_pairs(a_addr, b_addr, option::none(), 10,);
        assert!(res
            == vector[
                PairResponse {
                    coin_a: a_addr,
        ...
        let res =
            get_pairs(a_addr, b_addr, option::some(pair_1_addr), 10,);
        assert!(res
            == vector[
                PairResponse {
                    coin_a: a_addr,
