...
/// for blind signing.
module minitia_std::permissioned_signer {
    use std::signer;
    use std::error;
    use std::vector;
    use std::option::{Option, Self};
    use std::copyable_any::{Self, Any};
    use minitia_std::big_ordered_map::{Self, BigOrderedMap};
    use minitia_std::account::create_signer;
    use minitia_std::transaction_context::generate_unique_address;
    ...
    public fun create_permissioned_handle(master: &signer): PermissionedHandle {
        ...
        let permissions_storage_addr = generate_unique_address();
    ...
    public fun grant_revoke_permission(
        master: &signer, permissioned: &signer
    ...
    public entry fun revoke_permission_storage_address(
        ...
        let active_handles =
            &mut GrantedPermissionHandles[master_account_addr].active_handles;
    ...
    public entry fun revoke_all_handles(
        s: &signer
    ) acquires GrantedPermissionHandles, PermissionStorage {
        assert!(
            check_permission_exists(s, RevokePermissionHandlePermission {}),
    ...
    inline fun initialize_permission_address(
        permissions_storage_addr: address
    ) {
        move_to(
    ...
    public(package) fun create_storable_permissioned_handle(
        ...
        GrantedPermissionHandles[master_account_addr].active_handles.push_back(
    ...
    inline fun destroy_permissions_storage_address(
        ...
        if (exists<PermissionStorage>(permissions_storage_addr)) {
            ...
            big_ordered_map::destroy(perms, |_dv| {});
        }
    }
    ...
    public(package) fun check_permission_capacity_above<PermKey: copy + drop + store>(
        ...
        map_or(
            s,
            perm,
            |stored_permission| { is_above(stored_permission, threshold) },
            false
        )
    }
    ...
    public(package) fun check_permission_consume<PermKey: copy + drop + store>(
        ...
        map_or(
            s,
            perm,
            |stored_permission| { consume_capacity(stored_permission, threshold) },
            false
        )
    }
    ...
    #[test(creator = @0xcafe)]
    #[expected_failure(abort_code = 0x1C5, location = minitia_std::bcs)]
    fun signer_serialization(creator: &signer) acquires PermissionStorage {
        let aptos_framework = create_signer(@0x1);
        timestamp::set_time_has_started_for_testing(&aptos_framework);
        ...
        assert!(
            bcs::to_bytes(creator) == bcs::to_bytes(&signer::address_of(creator)),
            1
        );
