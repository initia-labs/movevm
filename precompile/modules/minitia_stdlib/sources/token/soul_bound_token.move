...
module minitia_std::soul_bound_token {
    ...
    public entry fun mint(
        ...
        mint_soul_bound_token_object(creator,
            collection,
            description,
            name,
            uri,
    ...
    inline fun borrow<T: key>(nft: Object<T>): &SoulBoundToken {
        ...
        assert!(exists<SoulBoundToken>(nft_address), error::not_found(ENFT_DOES_NOT_EXIST),);
    ...
    inline fun authorized_borrow<T: key>(nft: Object<T>, creator: &signer): &SoulBoundToken {
        ...
        assert!(nft::creator(nft) == signer::address_of(creator),
    ...
    public entry fun set_description<T: key>(
        ...
        assert!(is_mutable_description(nft), error::permission_denied(EFIELD_NOT_MUTABLE),);
    ...
    public entry fun add_property<T: key>(
        ...
        let soul_bound_token = authorized_borrow(nft, creator);
        assert!(are_properties_mutable(nft),
    ...
    inline fun borrow_collection<T: key>(nft: Object<T>): &SoulBoundTokenCollection {
        ...
        assert!(exists<SoulBoundTokenCollection>(collection_address),
    ...
    public fun is_mutable_collection_royalty<T: key>(collection: Object<T>,): bool acquires SoulBoundTokenCollection {
    ...
    public fun is_mutable_collection_nft_name<T: key>(collection: Object<T>,): bool acquires SoulBoundTokenCollection {
        borrow_collection(collection).mutable_nft_name
    }

    public fun is_mutable_collection_nft_uri<T: key>(collection: Object<T>,): bool acquires SoulBoundTokenCollection {
    ...
    public entry fun set_collection_description<T: key>(
        ...
        assert!(soul_bound_token_collection.mutable_description,
            error::permission_denied(EFIELD_NOT_MUTABLE),);
        collection::set_description(option::borrow(&soul_bound_token_collection.mutator_ref),
            description);
    }
    ...
    public fun set_collection_royalties<T: key>(
        ...
        let soul_bound_token_collection = authorized_borrow_collection(collection, creator);
        assert!(option::is_some(&soul_bound_token_collection.royalty_mutator_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),);
        royalty::update(option::borrow(&soul_bound_token_collection.royalty_mutator_ref),
            royalty);
    }
    ...
    public entry fun set_collection_uri<T: key>(
        ...
        let soul_bound_token_collection = authorized_borrow_collection(collection, creator);
        assert!(soul_bound_token_collection.mutable_uri,
            error::permission_denied(EFIELD_NOT_MUTABLE),);
    ...
    fun test_royalties(creator: &signer) acquires SoulBoundTokenCollection {
        ...
        set_collection_royalties_call(creator, collection, decimal128::from_ratio(2, 3), @
            0x444);
    ...
    #[test_only]
    fun mint_helper(
        creator: &signer, collection_name: String, nft_name: String, soul_bound_to: address
    ): Object<SoulBoundToken> acquires SoulBoundTokenCollection {
        mint_soul_bound_token_object(creator,
            collection_name,
