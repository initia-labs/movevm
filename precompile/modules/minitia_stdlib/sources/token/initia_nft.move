...
module minitia_std::initia_nft {
    ...
    fun mint_internal(
        ...
        let initia_nft =
            InitiaNft { burn_ref, mutator_ref, };
    ...
    inline fun borrow<T: key>(nft: Object<T>): &InitiaNft {
        ...
        assert!(exists<InitiaNft>(nft_address), error::not_found(ENFT_DOES_NOT_EXIST),);
    ...
    public entry fun burn<T: key>(owner: &signer, nft: Object<T>) acquires InitiaNft {
        let nft_address = object::object_address(nft);
        assert!(exists<InitiaNft>(nft_address), error::not_found(ENFT_DOES_NOT_EXIST),);
        assert!(object::owns(nft, signer::address_of(owner)),
    ...
    inline fun borrow_collection<T: key>(nft: Object<T>): &InitiaNftCollection {
        ...
        assert!(exists<InitiaNftCollection>(collection_address),
    ...
    public fun is_mutable_collection_royalty<T: key>(collection: Object<T>,): bool acquires InitiaNftCollection {
    ...
    public fun is_mutable_collection_nft_uri<T: key>(collection: Object<T>,): bool acquires InitiaNftCollection {
    ...
    public entry fun set_collection_description<T: key>(
        ...
        assert!(initia_nft_collection.mutable_description,
            error::permission_denied(EFIELD_NOT_MUTABLE),);
        collection::set_description(option::borrow(&initia_nft_collection.mutator_ref),
            description);
    }
    ...
    public fun set_collection_royalties<T: key>(
        ...
        let initia_nft_collection = authorized_borrow_collection(collection, creator);
        assert!(option::is_some(&initia_nft_collection.royalty_mutator_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),);
    ...
    public entry fun set_collection_uri<T: key>(
        ...
        let initia_nft_collection = authorized_borrow_collection(collection, creator);
        assert!(initia_nft_collection.mutable_uri,
            error::permission_denied(EFIELD_NOT_MUTABLE),);
