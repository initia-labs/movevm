...
module minitia_std::big_ordered_map {
    use std::error;
    use std::vector;
    ...
    use std::cmp;
    use std::storage_slots_allocator::{Self, StorageSlotsAllocator, StoredSlot};
    use std::math64::{max, min};
    ...
    public fun new_with_type_size_hints<K: store, V: store>(
        ...
        assert!(
            leaf_max_degree_from_max >= (INNER_MIN_DEGREE as u64),
            error::invalid_argument(EINVALID_CONFIG_PARAMETER)
        );
    ...
    public fun remove<K: drop + copy + store, V: store>(
        ...
        let Child::Leaf { value } = self.remove_at(path_to_leaf, key);
        value
    }
    ...
    public(friend) fun lower_bound<K: drop + copy + store, V: store>(
    ...
    public(friend) fun find<K: drop + copy + store, V: store>(
        self: &BigOrderedMap<K, V>, key: &K
    ): IteratorPtr<K> {
    ...
    public fun contains<K: drop + copy + store, V: store>(
        ...
        let lower_bound = self.lower_bound(key);
        if (lower_bound.iter_is_end(self)) { false }
        else if (&lower_bound.key == key) { true }
        else { false }
    }
    ...
    public(friend) fun new_begin_iter<K: copy + store, V: store>(
        self: &BigOrderedMap<K, V>
    ): IteratorPtr<K> {
        if (self.is_empty()) {
    ...
    public(friend) fun new_end_iter<K: copy + store, V: store>(
        self: &BigOrderedMap<K, V>
    ): IteratorPtr<K> {
        IteratorPtr::End
    }
    ...
    public(friend) fun iter_is_begin<K: store, V: store>(
    ...
    public(friend) fun iter_is_end<K: store, V: store>(
    ...
    /// Aborts with EITER_OUT_OF_BOUNDS if iterator is pointing to the end.
    /// Note: Requires that the map is not changed after the input iterator is generated.
    public(friend) fun iter_borrow<K: drop + store, V: store>(
        self: IteratorPtr<K>, map: &BigOrderedMap<K, V>
    ): &V {
    ...
    ///
    /// Note: Requires that the map is not changed after the input iterator is generated.
    public(friend) fun iter_borrow_mut<K: drop + store, V: store>(
        self: IteratorPtr<K>, map: &mut BigOrderedMap<K, V>
    ): &mut V {
        assert!(
            map.constant_kv_size,
            error::invalid_argument(EBORROW_MUT_REQUIRES_CONSTANT_KV_SIZE)
        );
        assert!(!self.iter_is_end(map), error::invalid_argument(EITER_OUT_OF_BOUNDS));
        let IteratorPtr::Some { node_index, child_iter, key: _ } = self;
    ...
    public(friend) fun iter_next<K: drop + copy + store, V: store>(
        self: IteratorPtr<K>, map: &BigOrderedMap<K, V>
    ): IteratorPtr<K> {
        assert!(
            !(self is IteratorPtr::End<K>),
    ...
    /// Requires the map is not changed after the input iterator is generated.
    public(friend) fun iter_prev<K: drop + copy + store, V: store>(
        self: IteratorPtr<K>, map: &BigOrderedMap<K, V>
    ): IteratorPtr<K> {
        let prev_index =
            if (self is IteratorPtr::End<K>) {
    ...
    fun validate_static_size_and_init_max_degrees<K: store, V: store>(
        ...
        if (key_size.is_some() && value_size.is_some()) {
            self.validate_size_and_init_max_degrees(
                key_size.destroy_some(), value_size.destroy_some()
            );
    ...
    fun new_inner_child<V: store>(node_index: StoredSlot): Child<V> {
        Child::Inner { node_index: node_index }
    }
    ...
    fun find_leaf<K: drop + copy + store, V: store>(
        self: &BigOrderedMap<K, V>, key: &K
    ): u64 {
        let current = ROOT_INDEX;
        loop {
    ...
    fun find_leaf_path<K: drop + copy + store, V: store>(
        self: &BigOrderedMap<K, V>, key: &K
    ): vector<u64> {
    ...
    fun add_at<K: drop + copy + store, V: store>(
        ...
        let (reserved_slot, node) =
            if (node_index == ROOT_INDEX) {
                ...
                let max_key = {
                    ...
                    let max_key =
                        *root_children.new_end_iter().iter_prev(root_children).iter_borrow_key(
                            root_children
                        );
        ...
        *left_next = right_node_index;
        ...
        let max_left_key =
            *left_children.new_end_iter().iter_prev(left_children).iter_borrow_key(
                left_children
            );
    ...
    fun update_key<K: drop + copy + store, V: store>(
        ...
        while (!path_to_node.is_empty()) {
            ...
            if (children.new_end_iter().iter_prev(children).iter_borrow_key(children)
                != &new_key) { return };
        }
    }
    ...
    fun remove_at<K: drop + copy + store, V: store>(
        ...
        let old_child = {
            ...
            let new_max_key =
                *children.new_end_iter().iter_prev(children).iter_borrow_key(children);
        ...
        let sibling_index = {
            let parent_children =
                &self.borrow_node(*path_to_node.borrow(path_to_node.length() - 1)).children;
            assert!(
        ...
        if ((sibling_children.length() - 1) * 2 >= max_degree) {
            ...
            if (sibling_index == next) {
                ...
                children.new_end_iter().iter_add(
                ...
                let sibling_end_iter =
                    sibling_children.new_end_iter().iter_prev(sibling_children);
                let borrowed_max_key =
                ...
                self.update_key(
                    path_to_node,
                    &borrowed_max_key,
                    *sibling_children.new_end_iter().iter_prev(sibling_children).iter_borrow_key(
                        sibling_children
                    )
                );
            };
        ...
        let slot_to_remove =
            self.remove_at(path_to_node, &key_to_remove).destroy_inner_child();
    ...
    fun print_map_for_node<K: store + copy + drop, V: store>(
        ...
        minitia_std::debug::print(&level);
        minitia_std::debug::print(&node_index);
        minitia_std::debug::print(node);
    ...
    #[test_only]
    fun destroy_and_validate<K: drop + copy + store, V: drop + store>(
        self: BigOrderedMap<K, V>
    ) {
        let it = self.new_begin_iter();
        while (!it.iter_is_end(&self)) {
            ...
            assert!(
                self.find(it.iter_borrow_key()).iter_is_end(&self),
                error::invalid_state(EINTERNAL_INVARIANT_BROKEN)
            );
        ...
        if (expected_lower_bound_key.is_some()) {
            ...
            assert!(
                cmp::compare(
                    ...
                    node.children.new_begin_iter().iter_borrow_key(&node.children)
                ).is_lt(),
                error::invalid_state(EINTERNAL_INVARIANT_BROKEN)
            );
        };
    ...
    #[test_only]
    fun validate_map<K: drop + copy + store, V: store>(
        self: &BigOrderedMap<K, V>
    ) {
    ...
    fun test_variable_size() {
        ...
        map.print_map();
        map.validate_map();
        map.add(vector[1], vector[1]);
        map.print_map();
        map.validate_map();
        map.add(vector[2], vector[2]);
        map.print_map();
        map.validate_map();
        let r1 = map.upsert(vector[3], vector[3]);
        map.print_map();
        map.validate_map();
        assert!(r1 == option::none(), 1);
        ...
        map.add(vector[5], vector[5]);
        map.print_map();
        map.validate_map();
        map.add(vector[6], vector[6]);
        map.print_map();
        map.validate_map();

        vector[1, 2, 3, 4, 5, 6].zip(
            vector[1, 2, 3, 8, 5, 6],
            |key, value| {
    ...
    #[test]
    #[expected_failure(abort_code = 0x10002, location = minitia_std::ordered_map)]
    /// EKEY_NOT_FOUND
    ...
    /// EBORROW_MUT_REQUIRES_CONSTANT_KV_SIZE
    ...
    inline fun comparison_test(
        ...
        for (i in 0..repeats) {
            ...
            if (is_insert) {
                let v = next_1();
                assert!(
                    big_map.upsert(v, v) == small_map.upsert(v, v),
                    i
                );
            } else {
                let v = next_2();
            ...
            if ((i + 1) % 50 == 0) {
                ...
                while (!big_iter.iter_is_end(&big_map)
                    || !small_iter.iter_is_end(&small_map)) {
                    assert!(
                        big_iter.iter_borrow_key()
                            == small_iter.iter_borrow_key(&small_map),
                        i
                    );
                    assert!(
                        big_iter.iter_borrow(&big_map)
                            == small_iter.iter_borrow(&small_map),
                        i
                    );
    ...
    #[test_only]
    fun test_large_data_set_helper(
        inner_max_degree: u16, leaf_max_degree: u16, reuse_slots: bool
    ) {
        use std::vector;
