...
///
module minitia_std::ordered_map {
    friend std::big_ordered_map;
    ...
    public(friend) fun iter_next<K, V>(
    ...
    /// Note: Requires that the map is not changed after the input iterator is generated.
    public(friend) fun iter_prev<K, V>(
        self: IteratorPtr, map: &OrderedMap<K, V>
    ): IteratorPtr {
    ...
    public(friend) fun iter_is_begin<K, V>(
        self: &IteratorPtr, map: &OrderedMap<K, V>
    ): bool {
        if (self is IteratorPtr::End) {
            map.is_empty()
    ...
    public(friend) fun iter_is_end<K, V>(
    ...
    /// Note: Requires that the map is not changed after the input iterator is generated.
    public(friend) fun iter_borrow_key<K, V>(
        self: &IteratorPtr, map: &OrderedMap<K, V>
    ): &K {
        assert!(
    ...
    /// Aborts with EITER_OUT_OF_BOUNDS if iterator is pointing to the end.
    /// Note: Requires that the map is not changed after the input iterator is generated.
    public(friend) fun iter_borrow<K, V>(
        self: IteratorPtr, map: &OrderedMap<K, V>
    ): &V {
        assert!(
            !(self is IteratorPtr::End), error::invalid_argument(EITER_OUT_OF_BOUNDS)
    ...
    /// Aborts with EITER_OUT_OF_BOUNDS if iterator is pointing to the end.
    /// Note: Requires that the map is not changed after the input iterator is generated.
    public(friend) fun iter_borrow_mut<K, V>(
    ...
    /// Aborts with EITER_OUT_OF_BOUNDS if iterator is pointing to the end.
    /// Note: Requires that the map is not changed after the input iterator is generated.
    public(friend) fun iter_remove<K: drop, V>(
        self: IteratorPtr, map: &mut OrderedMap<K, V>
    ): V {
        assert!(
            !(self is IteratorPtr::End), error::invalid_argument(EITER_OUT_OF_BOUNDS)
    ...
    fun print_map<K, V>(self: &OrderedMap<K, V>) {
        minitia_std::debug::print(&self.entries);
    }
    ...
    fun test_append() {
        ...
        };
        {
            ...
            map.append(other);
            assert!(
                map == new_from(vector[1, 2, 3, 4, 5], vector[10, 20, 30, 40, 50]),
                3
            );
        };
        {
    ...
    fun test_replace_key_inplace() {
        ...
        assert!(
            map == new_from(vector[1, 3, 6], vector[10, 30, 50]),
            1
        );
        map.replace_key_inplace(&3, 4);
        assert!(
            map == new_from(vector[1, 4, 6], vector[10, 30, 50]),
            2
        );
        map.replace_key_inplace(&1, 0);
        assert!(
            map == new_from(vector[0, 4, 6], vector[10, 30, 50]),
            3
        );
    }
    ...
    public fun test_iter_replace() {
        ...
        assert!(
            map == new_from(vector[1, 3, 5], vector[10, 35, 50]),
            1
        );
    }
    ...
    public fun test_iter_add() {
        {
            ...
            assert!(
                map == new_from(vector[0, 1, 3, 5], vector[5, 10, 30, 50]),
                1
            );
        };
        {
            ...
            assert!(
                map == new_from(vector[1, 2, 3, 5], vector[10, 20, 30, 50]),
                2
            );
        };
        {
            ...
            assert!(
                map == new_from(vector[1, 3, 5, 6], vector[10, 30, 50, 60]),
                3
            );
        };
        {
    ...
    public fun large_dataset(): vector<u64> {
        vector[
            ...
            540, 426, 172, 736, 211, 368, 567, 429, 782, 530, 862, 123, 67, 135, 929, 802,
            22, 58, 69, 167, 393, 456, 11, 42, 229, 373, 421, 919, 784, 537, 198, 324, 315,
            370, 413, 526, 91, 980, 956, 873, 862, 170, 996, 281, 305, 925, 84, 327, 336,
            505, 846, 729, 313, 857, 124, 895, 582, 545, 814, 367, 434, 364, 43, 750, 87,
            808, 276, 178, 788, 584, 403, 651, 754, 399, 932, 60, 676, 368, 739, 12, 226,
            586, 94, 539, 795, 570, 434, 378, 467, 601, 97, 902, 317, 492, 652, 756, 301,
            280, 286, 441, 865, 689, 444, 619, 440, 729, 31, 117, 97, 771, 481, 675, 709,
            927, 567, 856, 497, 353, 586, 965, 306, 683, 219, 624, 528, 871, 732, 829, 503,
            19, 270, 368, 708, 715, 340, 149, 796, 723, 618, 245, 846, 451, 921, 555, 379,
            488, 764, 228, 841, 350, 193, 500, 34, 764, 124, 914, 987, 856, 743, 491, 227,
            365, 859, 936, 432, 551, 437, 228, 275, 407, 474, 121, 858, 395, 29, 237, 235,
            793, 818, 428, 143, 11, 928, 529
        ]
    }
    ...
    public fun large_dataset_shuffled(): vector<u64> {
        vector[
            895, 228, 530, 784, 624, 335, 729, 818, 373, 456, 914, 226, 368, 750, 428, 956,
            437, 586, 763, 235, 567, 91, 829, 690, 434, 178, 584, 426, 228, 407, 237, 497,
            764, 135, 124, 421, 537, 270, 11, 367, 378, 856, 529, 276, 729, 618, 929, 227,
            149, 788, 925, 675, 121, 795, 306, 198, 421, 350, 555, 441, 403, 932, 368, 383,
            928, 841, 440, 771, 364, 902, 301, 987, 467, 873, 921, 11, 365, 340, 739, 492,
            540, 386, 919, 723, 539, 87, 12, 782, 324, 862, 689, 395, 488, 793, 709, 505,
            582, 814, 245, 980, 936, 736, 619, 69, 370, 545, 764, 886, 305, 551, 19, 865,
            229, 432, 29, 754, 34, 676, 43, 846, 451, 491, 871, 500, 915, 708, 586, 60,
            280, 652, 327, 172, 856, 481, 796, 474, 219, 651, 170, 281, 84, 97, 715, 857,
            353, 862, 393, 567, 368, 777, 97, 315, 526, 94, 31, 167, 123, 413, 503, 193,
            808, 649, 143, 42, 444, 317, 67, 926, 434, 211, 379, 570, 683, 965, 732, 927,
            429, 859, 313, 528, 996, 117, 492, 336, 22, 399, 275, 802, 743, 124, 846, 58,
            858, 286, 756, 601, 27, 59, 362, 793
        ]
    }
