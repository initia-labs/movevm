module std::bit_vector {
    ...
    spec fun spec_is_index_set(self: BitVector, bit_index: u64): bool {
        if (bit_index >= self.length()) { false }
        else {
            self.bit_field[bit_index]
        }
    }
        ...
        if (amount >= self.length) {
            ...
            while ({
                spec {
                    ...
                    invariant forall k in i..self.length:
            ...
            while ({
                spec {
                    ...
                    invariant forall j in (i - amount)..self.length:
            ...
            while ({
                spec {
                    ...
                    invariant forall k in 0..self.length - amount:
                        self.bit_field[k] == old(self).bit_field[k + amount];
    ...
    spec shift_left_for_verification_only {
        ...
        ensures amount >= self.length ==>
            (forall k in 0..self.length: !self.bit_field[k]);
        ensures amount < self.length ==>
            (forall i in self.length - amount..self.length: !self.bit_field[i]);
        ensures amount < self.length ==>
            (
                forall i in 0..self.length - amount:
