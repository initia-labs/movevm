module initia_std::vip_weight_vote {
    ...
    public entry fun vote_challenge(
        ...
        // emit event
        event::emit(VoteChallengeEvent { account: addr, challenge_id, voting_power, vote_option, })
    }
    ...
    fun execute_challenge_internal(challenge_id: u64): bool acquires ModuleStore {
        ...
        return apply_challenge(module_store,
            challenge.stage,
    ...
    fun submit_merkle_root_internal(
        ...
        table::add(&mut module_store.proposals,
            table_key::encode_u64(module_store.current_stage),
            Proposal {
                merkle_root,
                votes: table::new(),
    ...
    fun normalize_weights(
        ...
        vector::zip_reverse(bridge_ids,
            weights,
            |bridge_id, weight| {
                vector::push_back(&mut n_weights,
                    Weight {
                        bridge_id: bridge_id,
