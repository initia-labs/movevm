module initia_std::vip_vault {
    ...
    public(friend) fun claim(stage: u64,): FungibleAsset acquires ModuleStore {
        ...
        assert!(stage == module_store.claimable_stage,
            error::invalid_argument(EINVALID_STAGE));
        assert!(module_store.reward_per_stage > 0,
    ...
    public entry fun deposit(funder: &signer, amount: u64) acquires ModuleStore {
        ...
        primary_fungible_store::transfer(funder, vip_reward::reward_metadata(),
            vault_store_addr, amount);
    }
