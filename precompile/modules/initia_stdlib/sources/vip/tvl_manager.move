module initia_std::vip_tvl_manager {
    ...
    public(friend) fun add_snapshot(epoch: u64, bridge_id: u64, balance: u64) acquires ModuleStore {
        ...
        if (!table::contains(&module_store.average_tvl,table_key::encode_u64(epoch))) {
            table::add(&mut module_store.average_tvl, table_key::encode_u64(epoch), table::new<vector<u8>/*bridge id*/, table::Table<vector<u8>/*count*/, TVLSnapshot>>());
        };
        let average_tvl_epoch = table::borrow_mut(&mut module_store.average_tvl, table_key::encode_u64(epoch));
        if (!table::contains(average_tvl_epoch, table_key::encode_u64(bridge_id))) {
            table::add(average_tvl_epoch, table_key::encode_u64(bridge_id), table::new<vector<u8>/*count*/, TVLSnapshot>());
        };
        let average_tvl_table = table::borrow_mut(average_tvl_epoch, table_key::encode_u64(bridge_id));
        ...
        table::add(average_tvl_table,table_key::encode_u64(count), snapshot);
    }
    // calculate the average tvl of the bridge at the epoch from accumulated snapshots
    public fun calculate_average_tvl(epoch_key:vector<u8>,bridge_id_key:vector<u8>) :u64 acquires ModuleStore {
        let module_store = borrow_global_mut<ModuleStore>(@initia_std);
        if (!table::contains(&module_store.average_tvl,epoch_key))  {
            return 0
        };
        ...
        let iter = table::iter(tvl_by_bridge_id,option::none(),option::none(),1);
        let total_tvl = 0;
        loop {
            if(!table::prepare<vector<u8>,TVLSnapshot>(&mut iter)) {break};
            let (_,snapshot) = table::next<vector<u8>,TVLSnapshot>(&mut iter);
    ...
    public fun get_average_tvl(epoch: u64, bridge_id: u64):u64 acquires ModuleStore {
        calculate_average_tvl(table_key::encode_u64(epoch),table_key::encode_u64(bridge_id))
    }
    ...
    public fun get_snapshots(epoch: u64, bridge_id: u64): vector<TVLSnapshotResponse> acquires ModuleStore {
        ...
        loop {
            if (i >= count) break;
            let snapshot = table::borrow(average_tvl_table, table_key::encode_u64(i));
            vector::push_back(&mut snapshots, TVLSnapshotResponse {
                time: snapshot.time,
                tvl: snapshot.tvl,
            });
        };
    ...
    public fun add_snapshot_for_test(chain:&signer)acquires ModuleStore {
        ...
        add_snapshot(DEFAULT_EPOCH_FOR_TEST, DEFAULT_BRIDE_ID_FOR_TEST, balance);

        let average_tvl = calculate_average_tvl(table_key::encode_u64(DEFAULT_EPOCH_FOR_TEST),table_key::encode_u64(DEFAULT_BRIDE_ID_FOR_TEST));
    ...
    public fun add_multi_snapshot_for_test(chain:&signer)acquires ModuleStore {
        ...
        add_snapshot(DEFAULT_EPOCH_FOR_TEST, DEFAULT_BRIDE_ID_FOR_TEST, balance1);
        skip_period(DEFAULT_SKIP_FOR_TEST);
        add_snapshot(DEFAULT_EPOCH_FOR_TEST, DEFAULT_BRIDE_ID_FOR_TEST, balance2);
        skip_period(DEFAULT_SKIP_FOR_TEST);
        add_snapshot(DEFAULT_EPOCH_FOR_TEST, DEFAULT_BRIDE_ID_FOR_TEST, balance3);
        let average_tvl = calculate_average_tvl(table_key::encode_u64(DEFAULT_EPOCH_FOR_TEST),table_key::encode_u64(DEFAULT_BRIDE_ID_FOR_TEST));
