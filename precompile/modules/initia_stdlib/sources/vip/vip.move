module initia_std::vip {
    ...
    fun extract_commission(
        ...
        let commission_rate =
            vip_operator::get_operator_commission(operator_addr, bridge_id);
    ...
    fun split_reward(
        ...
        let initial_balance_pool_reward_amount =
            fungible_asset::amount(&balance_pool_reward);
        let initial_weight_pool_reward_amount =
        ...
        loop {
            ...
            total_operator_funded_reward = total_operator_funded_reward
                + fungible_asset::amount(&commission_sum);
            total_user_funded_reward = total_user_funded_reward
                + fungible_asset::amount(&user_reward_sum);
    ...
    fun calculate_balance_share(
        ...
        loop {
            ...
            let bridge_balance =
                vip_tvl_manager::calculate_average_tvl(
    ...
    public entry fun claim_operator_reward_script(
        ...
        if (!vip_vesting::is_operator_vesting_store_registered(
    ...
    public entry fun batch_claim_user_reward_script(
        ...
        assert!(
            vector::length(&stage) == vector::length(&merkle_proofs)
                && vector::length(&merkle_proofs) == vector::length(&l2_score),
            error::invalid_argument(EINVALID_BATCH_ARGUMENT),
        );
    ...
    public entry fun update_proportion(
        chain: &signer, proportion: Decimal256,
    ) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(signer::address_of(chain));
    ...
    entry public fun update_challenge_period(
        chain: &signer, challenge_period: u64,
    ) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(signer::address_of(chain));
    ...
    fun test_execute_challenge(
        ...
        assert!(create_time == expected_create_time, 0);
        ...
        assert!(expected_new_api_uri == new_api_uri, 0);
    ...
    fun test_shrink_reward(
        ...
        let remaining_reward_amount =
            vip_vesting::get_user_vesting_remaining_reward(
                signer::address_of(receiver),
                bridge_id,
                1,
            );
    ...
    fun test_fund_reward_script(chain: &signer, operator: &signer,) acquires ModuleStore {
        ...
        assert!(
            vip_reward::balance(vip_vesting::get_user_reward_store_address(1))
                == get_expected_reward(1, DEFAULT_REWARD_PER_STAGE_FOR_TEST),
            0,
        );
        assert!(
            vip_reward::balance(vip_vesting::get_user_reward_store_address(2))
                == get_expected_reward(2, DEFAULT_REWARD_PER_STAGE_FOR_TEST),
            0,
        );
        assert!(
            vip_reward::balance(vip_vesting::get_user_reward_store_address(3))
        ...
        fund_reward_script(chain, 2, release_time, release_time);
        assert!(
            vip_reward::balance(vip_vesting::get_operator_reward_store_address(1)) == 0,
            0,
        );
        assert!(
            vip_reward::balance(vip_vesting::get_operator_reward_store_address(2)) == 0,
            0,
        );
        assert!(
            vip_reward::balance(vip_vesting::get_operator_reward_store_address(3)) == 0,
            0,
        );
        ...
        fund_reward_script(chain, 3, release_time, release_time);
        assert!(
            vip_reward::balance(vip_vesting::get_operator_reward_store_address(1))
                == get_expected_reward(1, DEFAULT_REWARD_PER_STAGE_FOR_TEST) / 2,
            0,
        );
        assert!(
            vip_reward::balance(vip_vesting::get_operator_reward_store_address(2))
    ...
    fun test_e2e_scene1(
        ...
        assert!(
            coin::balance(
                ...
                + reward_per_stage / (vesting_period * 2) // stage 1
                    + reward_per_stage / (vesting_period * 2)
                    + reward_per_stage / (vesting_period * 2) // stage 2
        ...
        let user_reward_store_addr =
            vip_vesting::get_user_reward_store_address(bridge_id);
        let operator_reward_store_addr =
            vip_vesting::get_operator_reward_store_address(bridge_id);
        let bridge_info = get_bridge_info(bridge_id);
        assert!(
            bridge_info.user_reward_store_addr == user_reward_store_addr
    ...
    fun test_e2e_scene2(
        ...
        let user_claimed_stages =
            vip_vesting::get_user_claimed_stages(
                signer::address_of(receiver), bridge_id
            );
        let operator_claimed_stages =
            vip_vesting::get_operator_claimed_stages(
