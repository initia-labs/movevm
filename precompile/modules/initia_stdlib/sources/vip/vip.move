module initia_std::vip {
    ...
    fun init_module(chain: &signer) {
        move_to(chain,
            ModuleStore {
                ...
                pool_split_ratio: decimal256::from_string(&string::utf8(
                        DEFAULT_POOL_SPLIT_RATIO)),
                agent: signer::address_of(chain),
                maximum_tvl_ratio: decimal256::from_string(&string::utf8(
                        DEFAULT_MAXIMUM_TVL_RATIO)),
                minimum_eligible_tvl: DEFAULT_MINIMUM_ELIGIBLE_TVL,
                maximum_weight_ratio: decimal256::from_string(&string::utf8(
                        DEFAULT_MAXIMUM_WEIGHT_RATIO)),
    ...
    fun check_agent_permission(agent: &signer) acquires ModuleStore {
        ...
        assert!(signer::address_of(agent) == module_store.agent,
            error::permission_denied(EUNAUTHORIZED));
    }

    fun load_bridge(
        bridges: &table::Table<vector<u8>, Bridge>, bridge_id: u64
    ): &Bridge {
        assert!(table::contains(bridges, table_key::encode_u64(bridge_id)),
            error::not_found(EBRIDGE_NOT_FOUND));
        table::borrow(bridges, table_key::encode_u64(bridge_id))
    }

    fun load_bridge_mut(
        bridges: &mut table::Table<vector<u8>, Bridge>, bridge_id: u64
    ): &mut Bridge {
        assert!(table::contains(bridges, table_key::encode_u64(bridge_id)),
            error::not_found(EBRIDGE_NOT_FOUND));
    ...
    fun claim_user_reward(
        ...
        assert!(table::contains(&module_store.stage_data, table_key::encode_u64(stage)),
            error::not_found(ESTAGE_DATA_NOT_FOUND));
        let stage_data =
            table::borrow(&module_store.stage_data, table_key::encode_u64(stage));
        let snapshot =
            table::borrow(&stage_data.snapshots, table_key::encode_u64(bridge_id));
        assert!(block_time >= stage_data.user_vesting_release_time,
            error::unavailable(EVESTING_IN_PROGRESS));

        let target_hash =
            score_hash(bridge_id, stage, account_addr, l2_score, snapshot.total_l2_score,);

        assert_merkle_proofs(merkle_proofs, snapshot.merkle_root, target_hash,);

        let vested_reward =
            vip_vesting::claim_user_reward(account_addr,
                bridge_id,
                stage,
    ...
    fun zapping(
        ...
        let account_addr = signer::address_of(account);
        let esinit =
            vip_vesting::zapping_vesting(account_addr, bridge_id, stage, zapping_amount);
        assert!(primary_fungible_store::balance(account_addr, stakelisted_metadata) >= stakelisted_amount,
            error::invalid_argument(EZAPPING_STAKELISTED_NOT_ENOUGH));
        let stakelisted =
            primary_fungible_store::withdraw(account, stakelisted_metadata,
                stakelisted_amount);

        vip_zapping::zapping(account,
            bridge_id,
            lp_metadata,
            min_liquidity,
            validator,
    ...
    fun extract_commission(
        operator_addr: address, bridge_id: u64, reward: FungibleAsset,
    ): (FungibleAsset, FungibleAsset) {
        let commission_rate = vip_operator::get_operator_commission(operator_addr, bridge_id);
        let commission_amount =
            decimal256::mul_u64(&commission_rate, fungible_asset::amount(&reward));
    ...
    fun split_reward(
        ...
        let index = 0;
        let iter =
            table::iter(&module_store.bridges, option::none(), option::none(), 1);
        loop {
            ...
            let balance_reward =
                split_reward_with_share(balance_shares, bridge_id,
                    initial_balance_pool_reward_amount, &mut balance_pool_reward);
            let (balance_commission, balance_user_reward) =
                extract_commission(bridge.operator_addr, bridge_id, balance_reward);

            let weight_reward =
                split_reward_with_share(weight_shares, bridge_id,
                    initial_weight_pool_reward_amount, &mut weight_pool_reward);
            let (weight_commission, weight_user_reward) =
                extract_commission(bridge.operator_addr, bridge_id, weight_reward);
            ...
            vector::push_back(&mut reward_distributions,
                RewardDistribution {
                    bridge_id,
    ...
    fun split_reward_with_share(
        ...
        let split_amount =
            split_reward_with_share_internal(shares, bridge_id, total_reward_amount);
    ...
    fun fund_reward(
        module_store: &mut ModuleStore, stage: u64, initial_reward: FungibleAsset
    ): (u64, u64) {
        ...
        let balance_pool_reward =
            fungible_asset::extract(&mut initial_reward, balance_pool_reward_amount);
        let weight_pool_reward = initial_reward;

        let (total_operator_funded_reward, total_user_funded_reward) =
            split_reward(module_store,
                stage,
                &balance_shares,
                &weight_shares,
                balance_pool_reward,
    ...
    fun bridge_total_balance(module_store: &ModuleStore,): u64 {
        let total_balance = 0;
        ...
        loop {
            ...
            let bridge_balance =
                primary_fungible_store::balance(bridge.bridge_addr,
    ...
    fun calculate_balance_share(
        module_store: &ModuleStore, balance_shares: &mut SimpleMap<u64, Decimal256>
    ): u64 {
        let total_balance = bridge_total_balance(module_store);
        let max_effective_balance =
            decimal256::mul_u64(&module_store.maximum_tvl_ratio, total_balance);
        ...
        loop {
            ...
            let effective_bridge_balance =
                if (bridge_balance > max_effective_balance) { max_effective_balance }
                else if (bridge_balance < module_store.minimum_eligible_tvl) { 0 }
                else { bridge_balance };

            let share =
                decimal256::from_ratio_u64(effective_bridge_balance, total_balance);
    ...
    fun calculate_weight_share(
        module_store: &ModuleStore, weight_shares: &mut SimpleMap<u64, Decimal256>
    ) {
        let iter =
            table::iter(&module_store.bridges, option::none(), option::none(), 1);
        loop {
            ...
            let weight =
                if (decimal256::val(&bridge.vip_weight) > decimal256::val(&module_store.maximum_weight_ratio)) {
    ...
    fun claim_operator_reward(operator: &signer, bridge_id: u64, stage: u64,): FungibleAsset acquires ModuleStore {
        ...
        let stage_data =
            table::borrow(&module_store.stage_data, table_key::encode_u64(stage));
        assert!(block_time >= stage_data.operator_vesting_release_time,
            error::unavailable(EVESTING_IN_PROGRESS));

        let vested_reward =
            vip_vesting::claim_operator_reward(operator_addr,
                bridge_id,
                stage,
    ...
    fun validate_vip_weights(module_store: &ModuleStore) {
        ...
        assert!(decimal256::val(&total_weight) <= decimal256::val(&decimal256::one()),
    ...
    public(friend) fun update_vip_weights_for_friend(
        ...
        assert!(vector::length(&bridge_ids) == vector::length(&weights),
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));

        vector::enumerate_ref(&bridge_ids,
            |i, id| {
    ...
    public entry fun register(
        ...
        assert!(!table::contains(&module_store.bridges, table_key::encode_u64(bridge_id)),
        ...
        if (!vip_operator::is_operator_store_registered(operator, bridge_id)) {
            vip_operator::register_operator_store(chain,
                operator,
                bridge_id,
                module_store.stage,
        ...
        // add bridge info
        table::add(&mut module_store.bridges,
            table_key::encode_u64(bridge_id),
            Bridge {
                bridge_addr: bridge_address,
                operator_addr: operator,
                vip_weight: decimal256::zero(),
                user_reward_store_addr: vip_vesting::get_user_reward_store_address(
                    bridge_id),
                operator_reward_store_addr: vip_vesting::get_operator_reward_store_address(
                    bridge_id),
            });
    }
    ...
    public entry fun update_agent(old_agent: &signer, new_agent: address,) acquires ModuleStore {
    ...
    public entry fun fund_reward_script(
        ...
        let module_store = borrow_global_mut<ModuleStore>(@initia_std);
        assert!(!table::contains(&mut module_store.stage_data, table_key::encode_u64(stage)),
        ...
        // set stage data
        let module_store = borrow_global_mut<ModuleStore>(@initia_std);
        table::add(&mut module_store.stage_data,
    ...
    public entry fun submit_snapshot(
        ...
        assert!(table::contains(&module_store.stage_data, table_key::encode_u64(stage)),
            error::not_found(ESTAGE_DATA_NOT_FOUND));
        let stage_data =
            table::borrow_mut(&mut module_store.stage_data, table_key::encode_u64(stage));

        assert!(!table::contains(&stage_data.snapshots, table_key::encode_u64(bridge_id)),
            error::already_exists(ESNAPSHOT_ALREADY_EXISTS));
        table::add(&mut stage_data.snapshots,
            table_key::encode_u64(bridge_id),
            Snapshot { merkle_root, total_l2_score, });
    }
    ...
    public entry fun update_snapshot(
        ...
        let snapshot =
            table::borrow_mut(&mut stage_data.snapshots, table_key::encode_u64(bridge_id));
        snapshot.merkle_root = merkle_root;
        snapshot.total_l2_score = total_l2_score;
    }

    public entry fun claim_operator_reward_script(
        operator: &signer, bridge_id: u64, stage: u64,
    ) acquires ModuleStore {
        if (!vip_vesting::is_operator_vesting_store_registered(signer::address_of(operator),
                bridge_id)) {
            vip_vesting::register_operator_vesting_store(operator, bridge_id);
        };
        let vested_reward =
            claim_operator_reward(operator, bridge_id, stage,);
    ...
    public entry fun claim_user_reward_script(
        ...
        let vested_reward =
            claim_user_reward(account, bridge_id, stage, merkle_proofs, l2_score,);

        coin::deposit(signer::address_of(account), vested_reward);
    }

    public entry fun batch_claim_operator_reward_script(
        operator: &signer, bridge_id: u64, stage: vector<u64>,
    ) acquires ModuleStore {
        let prev_stage = 0;
    ...
    public entry fun update_vip_weights(
        chain: &signer, bridge_ids: vector<u64>, weights: vector<Decimal256>,
    ) acquires ModuleStore {
        check_chain_permission(chain);
    ...
    public entry fun update_vip_weight(
        chain: &signer, bridge_id: u64, weight: Decimal256,
    ) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(@initia_std);
    ...
    public entry fun update_vesting_period(
        chain: &signer, user_vesting_period: u64, operator_vesting_period: u64,
    ) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(signer::address_of(chain));
        assert!(user_vesting_period > 0 && operator_vesting_period > 0,
    ...
    public entry fun update_minimum_eligible_tvl(
        chain: &signer, minimum_eligible_tvl: u64,
    ) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(signer::address_of(chain));
    ...
    public entry fun update_maximum_tvl_ratio(
        chain: &signer, maximum_tvl_ratio: Decimal256,
    ) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(signer::address_of(chain));
        assert!(decimal256::val(&maximum_tvl_ratio) <= decimal256::val(&decimal256::one()),
            error::invalid_argument(EINVALID_MAX_TVL));
        module_store.maximum_tvl_ratio = maximum_tvl_ratio;
    }

    public entry fun update_proportion(chain: &signer, proportion: Decimal256,) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(signer::address_of(chain));
    ...
    public entry fun update_pool_split_ratio(
        chain: &signer, pool_split_ratio: Decimal256,
    ) acquires ModuleStore {
        check_chain_permission(chain);
        let module_store = borrow_global_mut<ModuleStore>(signer::address_of(chain));
        assert!(decimal256::val(&pool_split_ratio) <= decimal256::val(&decimal256::one()),
    ...
    public entry fun batch_zapping_script(
        ...
        assert!(vector::length(&lp_metadata) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        assert!(vector::length(&min_liquidity) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        assert!(vector::length(&validator) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        assert!(vector::length(&zapping_amount) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        assert!(vector::length(&stakelisted_amount) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        assert!(vector::length(&stakelisted_metadata) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));

        vector::enumerate_ref(&stage,
            |i, s| {
                zapping(account,
                    bridge_id,
    ...
    public entry fun update_operator_commission(
        ...
        let module_store = borrow_global<ModuleStore>(@initia_std);
        vip_operator::update_operator_commission(operator, bridge_id, module_store.stage,
            commission_rate);
    }
    ...
    public fun get_snapshot(bridge_id: u64, stage: u64): SnapshotResponse acquires ModuleStore {
        ...
        let snapshots =
            table::borrow(&module_store.stage_data, table_key::encode_u64(stage));
        assert!(table::contains(&snapshots.snapshots, table_key::encode_u64(bridge_id)),
            error::not_found(ESNAPSHOT_NOT_EXISTS));
        let snapshot =
            table::borrow(&snapshots.snapshots, table_key::encode_u64(bridge_id));
    ...
    public fun get_expected_reward(bridge_id: u64, fund_reward_amount: u64): u64 acquires ModuleStore {
        ...
        let weight_ratio =
            decimal256::sub(&decimal256::one(), &module_store.pool_split_ratio);
        let balance_pool_reward_amount =
            decimal256::mul_u64(&module_store.pool_split_ratio, fund_reward_amount);
        let weight_pool_reward_amount = decimal256::mul_u64(&weight_ratio,
            fund_reward_amount);

        let balance_split_amount =
            split_reward_with_share_internal(&balance_shares, bridge_id,
                balance_pool_reward_amount);
        let weight_split_amount =
            split_reward_with_share_internal(&weight_shares, bridge_id,
    ...
    public fun get_next_stage(bridge_id: u64): u64 acquires ModuleStore {
        ...
        let iter =
            table::iter(&module_store.stage_data, option::none(), option::none(), 2);
        loop {
    ...
    public fun batch_simulate_user_claim_reward(
        ...
        assert!(vector::length(&minimum_score) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        assert!(vector::length(&vesting_period) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        assert!(vector::length(&l2_scores) == batch_length,
            error::invalid_argument(EINVALID_BATCH_ARGUMENT));
        ...
        vector::enumerate_ref(&initial_reward,
            |i, reward| {
                let (claimed_reward, remaining_reward) =
                    simulate_user_claim_reward(*reward,
    ...
    public fun simulate_user_claim_reward(
        initial_reward: u64, minimum_score: u64, vesting_period: u64, l2_scores: vector<u64>
    ): (u64, u64) {
        let total_claimed_reward = 0;
        let remaining_reward = initial_reward;
        vector::enumerate_ref(&l2_scores,
            |_i, l2_score| {
                let score_ratio =
    ...
    public fun test_setup(
        ...
        let (burn_cap, freeze_cap, mint_cap, _) = initialize_coin(chain,
            string::utf8(b"uinit"));
    ...
    public fun merkle_root_and_proof_scene1()
        ...
        simple_map::add(&mut root_map, 1,
            x"fb9eab6b9b5f195d0927c8a7301682b1475425249bb6b8bb31afd0dbb2dd4d09");
        simple_map::add(&mut root_map, 2,
            x"0ac37a58eb526e4577e78f59c46e70b3d0fd54b78c06905345bd7e14e75da42b");
        simple_map::add(&mut root_map, 3,
            x"42c600b41e6ff29ee44e1d61d460f6c78db862c0f3abe42d14df858649a1eea9");
        simple_map::add(&mut root_map, 4,
            x"dda4a2cd3385326bb304d1a6a62c35d39bb28d5acef58b5552e73b3c968e0c79");
        simple_map::add(&mut root_map, 5,
            x"469bdc31f3b0fbc1fb1f2ab9337af4ecf1643d6173cdecee95b235c9ca232017");
        simple_map::add(&mut root_map, 6,
            x"d2197ca826f0ee6084555f86fdd185a16788d68d8c512b025cb5829770682bd7");
        simple_map::add(&mut root_map, 7,
            x"998d5df26676a108e6581d1bc6dab1c7fab86fbdbcc5f1b8e4847ebe74f29341");
        simple_map::add(&mut root_map, 8,
            x"c41ff3aa918e489fc64a62d07915dab0c04b205e05dc6c9e4a8b7997091fdbdc");
        simple_map::add(&mut root_map, 9,
            x"c363c5b4393942032b841d5d0f68213d475e285b2fd7e31a4128c97b91cef97a");
        simple_map::add(&mut root_map, 10,
            x"2c4cc1daece91ee14d55d35595d17b8cc0bd6741b967ff82f73f6330c8b25b8a");

        simple_map::add(&mut proofs_map,
            1,
            vector[x"0bb9c560686ab3b4e1ac1a41bbc74ccd4d348634985a1a312590346900a6c93e"]);
        simple_map::add(&mut proofs_map,
            2,
            vector[x"66ffc3bb14e3bc65e022401feed6e2644082ccf69ccb40d1842fc6ca2d4c24fd"]);
        simple_map::add(&mut proofs_map,
            3,
            vector[x"70ed0c868798b88361b42895df358f64c4b4dd074f0af7146ef8898a675fee4e"]);
        simple_map::add(&mut proofs_map,
            4,
            vector[x"3e304abd07a33f4fab39537a4ac75c8886a89be9d8aaa96035675775a784b23e"]);
        simple_map::add(&mut proofs_map,
            5,
            vector[x"2911095fa7f35a563471cfff4135031f5d648372cc384b6288a19d8216baa3fa"]);
        simple_map::add(&mut proofs_map,
            6,
            vector[x"25a20d529493d2aef8beef43221b00231a0e8d07990e3d43b93fbf9cfd54de73"]);
        simple_map::add(&mut proofs_map,
            7,
            vector[x"61a55e6aac46c32a47c96b0dc4fd5de1f705e7400460957acb10457904a4a990"]);
        simple_map::add(&mut proofs_map,
            8,
            vector[x"96187ed75a9b83537e045912573bf3efee0a6369a663f1cb4d4ec7798c9f6299"]);
        simple_map::add(&mut proofs_map,
            9,
            vector[x"759ac8ad2821f2dbeb253e0872c07ffc6ccd3f69b80d19b04f0e49d6a0ea8da7"]);
        simple_map::add(&mut proofs_map,
            10,
            vector[x"98b1fed6531d027c0efb53d54941c83f8ceb9694b9ec199ee07278200c943eb1"]);
    ...
    public fun test_setup_scene1(
        agent: &signer, bridge_id: u64, release_time: u64,
    ) acquires ModuleStore {
        let idx = 1;
    ...
    fun test_update_proportion(
        chain: &signer, operator: &signer, receiver: &signer
    ) acquires ModuleStore {
        let bridge_id =
            test_setup(chain, operator, 1, @0x99, 1_000_000_000_000,);
        let release_time = 0;
        ...
        claim_user_reward_script(receiver,
            bridge_id,
            1,
        ...
        assert!(vip_vesting::get_user_vesting_minimum_score(signer::address_of(receiver),
                bridge_id, 1)
            == *simple_map::borrow(&score_map, &1),
            1);
        assert!(vip_vesting::get_user_vesting_minimum_score(signer::address_of(receiver),
    ...
    fun test_get_last_claimed_stages(
        ...
        claim_user_reward_script(receiver,
            ...
            *simple_map::borrow(&merkle_proof_map, &1),
            *simple_map::borrow(&score_map, &1));
        assert!(vip_vesting::get_user_last_claimed_stage(signer::address_of(receiver),
                bridge_id) == 1, 1);
    ...
    fun test_update_vesting_period(
        ...
        let expected_reward =
            (reward_per_stage / vesting_period + reward_per_stage / (vesting_period * 2) + reward_per_stage
                / (vesting_period * 2) + reward_per_stage / vesting_period // stage 1
    ...
    fun test_batch_claim(chain: &signer, operator: &signer, receiver: &signer) acquires ModuleStore {
        ...
        batch_claim_user_reward_script(receiver,
            bridge_id,
            vector[1, 2, 3, 4, 5, 6],
            vector[
                *simple_map::borrow(&merkle_proof_map, &1), *simple_map::borrow(&merkle_proof_map, &
                    2), *simple_map::borrow(&merkle_proof_map, &3), *simple_map::borrow(&merkle_proof_map, &
                        4), *simple_map::borrow(&merkle_proof_map, &5), *simple_map::borrow(
                    &merkle_proof_map, &6),],
            vector[1_000, 1_000, 500, 500, 100, 100],);

        batch_claim_operator_reward_script(receiver, bridge_id, vector[1, 2, 3, 4, 5, 6]);
    }
    ...
    fun test_shrink_reward(
        ...
        let initia_reward_amount =
            vip_vesting::get_user_vesting_initial_reward(signer::address_of(receiver),
                bridge_id, 1);

        let remaining_reward_amount =
            vip_vesting::get_user_vesting_remaining_reward(signer::address_of(receiver),
                bridge_id, 1);
        ...
        assert!(reward_by_stage_1
            == max_reward_per_claim // score_ratio = 1
    ...
    fun test_fund_reward_script(chain: &signer, operator: &signer,) acquires ModuleStore {
        ...
        assert!(get_expected_reward(1, DEFAULT_REWARD_PER_STAGE_FOR_TEST) ==
            35_000_000_000, 0); // 0.7 * DEFAULT_REWARD_PER_STAGE_FOR_TEST * (2/4) + 0.3 * DEFAULT_REWARD_PER_STAGE_FOR_TEST * 0
        assert!(get_expected_reward(2, DEFAULT_REWARD_PER_STAGE_FOR_TEST) ==
            17_500_000_000, 0); // 0.7 * DEFAULT_REWARD_PER_STAGE_FOR_TEST * (1/4) + 0.3 * DEFAULT_REWARD_PER_STAGE_FOR_TEST * 0
        assert!(get_expected_reward(3, DEFAULT_REWARD_PER_STAGE_FOR_TEST) ==
            17_500_000_000, 0); // 0.7 * DEFAULT_REWARD_PER_STAGE_FOR_TEST * (1/4) + 0.3 * DEFAULT_REWARD_PER_STAGE_FOR_TEST * 0

        assert!(vip_reward::balance(vip_vesting::get_user_reward_store_address(1))
            == get_expected_reward(1, DEFAULT_REWARD_PER_STAGE_FOR_TEST),
            0);
        assert!(vip_reward::balance(vip_vesting::get_user_reward_store_address(2))
            == get_expected_reward(2, DEFAULT_REWARD_PER_STAGE_FOR_TEST),
            0);
        assert!(vip_reward::balance(vip_vesting::get_user_reward_store_address(3))
            == get_expected_reward(3, DEFAULT_REWARD_PER_STAGE_FOR_TEST),
            0);

        fund_reward_script(chain, 2, release_time, release_time);
        assert!(vip_reward::balance(vip_vesting::get_operator_reward_store_address(1)) ==
            0, 0);
        assert!(vip_reward::balance(vip_vesting::get_operator_reward_store_address(2)) ==
            0, 0);
        assert!(vip_reward::balance(vip_vesting::get_operator_reward_store_address(3)) ==
            0, 0);

        update_operator_commission(operator, 1,
            decimal256::from_string(&string::utf8(b"0.5")));
        update_operator_commission(operator, 2,
            decimal256::from_string(&string::utf8(b"0.5")));
        fund_reward_script(chain, 3, release_time, release_time);
    ...
    fun test_deregistered_bridge(
        chain: &signer, agent: &signer, operator: &signer, receiver: &signer
    ...
    #[test(chain = @0x1, operator = @0x56ccf33c45b99546cd1da172cf6849395bbf8573, receiver = @0x19c9b6007d21a996737ea527f46b160b0a057c37)]
    fun test_e2e_scene1(chain: &signer, operator: &signer, receiver: &signer) acquires ModuleStore {
        let operator_addr = signer::address_of(operator);
        ...
        assert!(coin::balance(signer::address_of(receiver), vip_reward::reward_metadata())
            == (reward_per_stage / vesting_period + reward_per_stage / (vesting_period * 2)
                + reward_per_stage / (vesting_period * 2) // stage 1
                + reward_per_stage / (vesting_period * 2) + reward_per_stage / (
                    vesting_period * 2
                ) // stage 2
        ...
        let proportion =
            decimal256::from_string(&string::utf8(DEFAULT_PROPORTION_RATIO_FOR_TEST));
        let (claimable_list, _) =
            batch_simulate_user_claim_reward(vector[reward_per_stage, reward_per_stage],
                vector[
        ...
        assert!(claimable_v1
            == (reward_per_stage / vesting_period + reward_per_stage / (vesting_period * 2)
                + reward_per_stage / (vesting_period * 2) + reward_per_stage / vesting_period),
            0);
        assert!(claimable_v2
            == (reward_per_stage / (vesting_period * 2) + reward_per_stage / (
                    vesting_period * 2
        ...
        assert!(bridge_info.user_reward_store_addr == user_reward_store_addr
            && bridge_info.operator_reward_store_addr
            == operator_reward_store_addr
            && bridge_info.operator_addr == operator_addr
            && bridge_info.bridge_addr == @0x99
            && decimal256::is_same(&bridge_info.vip_weight,
                &decimal256::from_string(&string::utf8(DEFAULT_VIP_WEIGHT_RATIO_FOR_TEST))),
            7);
        assert!(vip_reward::get_stage_reward(user_reward_store_addr, 1)
            == total_reward_per_stage, 8);
    ...
    fun test_e2e_scene2(
        ...
        claim_user_reward_script(receiver,
            ...
            *simple_map::borrow(&merkle_proof_map, &1),
            *simple_map::borrow(&score_map, &1));
        assert!(vip_vesting::get_user_locked_reward(signer::address_of(receiver), bridge_id,
                    1) == reward_per_stage,
            0);
        assert!(vip_vesting::get_user_unlocked_reward(signer::address_of(receiver),
                bridge_id,
                1,
                *simple_map::borrow(&score_map, &1)) == 0,
            0);
        ...
        assert!(vip_vesting::get_user_unlocked_reward(signer::address_of(receiver),
            ...
            0);
        assert!(vip_vesting::get_user_vesting_initial_reward(signer::address_of(receiver),
                bridge_id, 4)
            == (reward_per_stage - decimal256::mul_u64(&commission_rate, reward_per_stage)),
            0);
        assert!(vip_vesting::get_operator_unlocked_reward(signer::address_of(operator),
                bridge_id, 4) == 0, 0);
        assert!(vip_vesting::get_operator_vesting_initial_reward(signer::address_of(operator),
                bridge_id, 4)
        ...
        assert!(vip_vesting::get_user_unlocked_reward(signer::address_of(receiver),
                ...
                *simple_map::borrow(&score_map, &5))
            == (reward_per_stage_by_vesting + (reward_per_stage_by_vesting / 2) + reward_per_stage_by_vesting
                + decimal256::mul_u64(&decimal256::from_string(&string::utf8(b"0.97")),
                    reward_per_stage_by_vesting)),
            0);
        assert!(vip_vesting::get_operator_unlocked_reward(signer::address_of(operator),
                bridge_id, 5)
        ...
        let user_claimed_stages =
            vip_vesting::get_user_claimed_stages(signer::address_of(receiver), bridge_id);
        let operator_claimed_stages =
            vip_vesting::get_operator_claimed_stages(signer::address_of(operator), bridge_id);
    ...
    fun test_update_snapshot(chain: &signer, operator: &signer) acquires ModuleStore {
        let bridge_id =
            test_setup(chain, operator, 1, @0x1111, 10000000000000000,);
    ...
    fun test_get_next_stage(
        chain: &signer, operator: &signer, operator2: &signer
    ) acquires ModuleStore, TestCapability {
        let bridge_id =
            test_setup(chain, operator, 1, @0x1111, 10000000000000000,);
        let release_time = 0;
    ...
    public fun test_setup_for_zapping(
        ...
        let (_burn_cap, _freeze_cap, mint_cap, _) = initialize_coin(chain,
            string::utf8(b"uinit"));
        ...
        let (_burn_cap, _freeze_cap, mint_cap, stakelisted_metadata) = initialize_coin(chain,
            string::utf8(b"USDC"));
        coin::mint_to(&mint_cap, signer::address_of(chain), mint_amount);
        coin::mint_to(&mint_cap, signer::address_of(account), mint_amount);

        dex::create_pair_script(chain,
            string::utf8(b"pair"),
        ...
        let lp_metadata =
            coin::metadata(signer::address_of(chain), string::utf8(b"INIT-USDC"));
    ...
    fun test_zapping(
        chain: &signer, operator: &signer, receiver: &signer, relayer: &signer,
    ) acquires ModuleStore {
        let mint_amount = 10_000_000_000_000;
        let (bridge_id, reward_metadata, stakelisted_metadata, lp_metadata, validator) =
            test_setup_for_zapping(chain, operator, receiver, 1, @0x99, mint_amount,);
        ...
        let zapping_amount =
            vip_vesting::get_user_locked_reward(signer::address_of(receiver), bridge_id,
                stage);

        // zap vesting in stage 1
        zapping_script(receiver,
            bridge_id,
            lp_metadata,
            option::none(),
            validator,
        ...
        let before_balance =
            primary_fungible_store::balance(signer::address_of(receiver), reward_metadata);
    ...
    fun test_full_vesting_zapping(
        ...
        let vesting_period = 10;
        let (bridge_id, _reward_metadata, stakelisted_metadata, lp_metadata, validator) =
            test_setup_for_zapping(chain, operator, receiver, 1, @0x99, 1_000_000_000_000,);
        let idx = 1;
