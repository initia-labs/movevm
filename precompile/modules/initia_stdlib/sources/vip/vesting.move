module initia_std::vip_vesting {
    ...
    fun get_vesting_finalized<Vesting: copy + drop + store>(
        ...
        assert!(
            ...
            error::not_found(EVESTING_NOT_FOUND));
        let vesting_finalized = table::borrow(&vesting_store.vestings_finalized,
            table_key::encode_u64(stage));
    ...
    fun get_last_claimed_stage<Vesting: copy + drop + store>(
        ...
        let iter = table::iter(&mut vesting_store.claimed_stages, option::none(),
            option::none(), 2);
    ...
    fun vest_user_reward(
        ...
        let vesting_store_addr =
            get_vesting_store_address<UserVesting>(account_addr, bridge_id);
        let vesting_store = borrow_global_mut<VestingStore<UserVesting>>(vesting_store_addr);
        let iter = table::iter_mut(&mut vesting_store.vestings, option::none(),
            option::none(), 1);
        loop {
        ...
        vector::for_each_ref(
            ...
            |stage| {
                ...
                let vesting = table::remove(&mut vesting_store.vestings,
                    table_key::encode_u64(*stage));
    ...
    fun vest_operator_reward(
        ...
        let finalized_vestings = vector::empty<u64>();
        let vesting_changes = vector::empty<VestingChange>();
        let vesting_store_addr = get_vesting_store_address<OperatorVesting>(account_addr,
            bridge_id);
    ...
    fun claim_previous_user_vestings(
        ...
        // vest previous vesting rewards until the stage
        let (amount, vesting_changes) = vest_user_reward(account_addr, bridge_id, stage,
            l2_score,);
    ...
    public(friend) fun claim_user_reward(
        ...
        let (vested_reward, vesting_changes) = claim_previous_user_vestings(account_addr,
            bridge_id, start_stage, l2_score,);
    ...
    public(friend) fun claim_operator_reward(
        ...
        let (vested_reward, vesting_changes) = claim_previous_operator_vestings(
            account_addr, bridge_id, start_stage,);

        let vesting_reward_amount = add_operator_vesting(account_addr, bridge_id,
    ...
    public(friend) fun zapping_vesting(
        ...
        let vesting = table::borrow_mut(&mut vesting_store.vestings,
            table_key::encode_u64(stage));
    ...
    public fun get_user_locked_reward(
        ...
        let vesting_store_addr =
            get_vesting_store_address<UserVesting>(account_addr, bridge_id);
        let vesting_store = borrow_global_mut<VestingStore<UserVesting>>(vesting_store_addr);
        let iter = table::iter(&mut vesting_store.vestings, option::none(),
