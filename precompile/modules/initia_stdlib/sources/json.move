module initia_std::json {
    ...
    fun stringify_internal(
        json_obj: &JsonObject,
        current_index: JsonIndex
        ...
        if (type == JSON_VALUE_TYPE_BOOL) {
            (
                json_elem.key,
                stringify_bool(as_bool(json_elem.value))
            )
        }
        else if (type == JSON_VALUE_TYPE_NUMBER) {
            (
                json_elem.key,
                stringify_number(as_number(json_elem.value))
            )
        }
        else if (type == JSON_VALUE_TYPE_STRING) {
            (
                json_elem.key,
                stringify_string(as_string(json_elem.value))
            )
        }
    ...
    fun parse_internal(
        json_obj: &mut JsonObject,
        type: u8, key: Option<String>,
        json_string: String,
        current_index: JsonIndex
    ) {
        assert!(
    ...
    public fun is_null_index(index: &JsonIndex): bool {
        if (vector::length(&index.data) == 0) { true } else { false }
    }
    ...
    fun set_elem(
        ...
        assert!(
            !simple_map::contains_key(&object.data, &index),
            EDUPLICATED_INDEX
        );
    ...
    public fun set_bool(
        object: &mut JsonObject,
        index: JsonIndex, key: Option<String>,
        value: bool
    ) {
        set_elem(
            object,
    ...
    public fun as_bool(json_value: JsonValue): bool {
        assert!(
            json_value.type == JSON_VALUE_TYPE_BOOL,
            ETYPE_MISMATCH
        );
    ...
    public fun as_dec(json_value: JsonValue): (bool, Decimal256) { // (signed, abs_val)
        let number = as_number(json_value);
        assert!(
            number.type == NUMBER_TYPE_DEC,
            error::invalid_argument(ETYPE_MISMATCH)
        );
        (
            number.is_positive,
            decimal256::new(number.value)
        )
    }
    ...
    fun test_get_type() {
        assert!(
            get_type(&string::utf8(b"1234")) == JSON_VALUE_TYPE_NUMBER,
            0
        );
        assert!(
            get_type(
        ...
        assert!(
            get_type(&string::utf8(b"true")) == JSON_VALUE_TYPE_BOOL,
            2
        );
        assert!(
            get_type(&string::utf8(b"\"true\"")) == JSON_VALUE_TYPE_STRING,
            3
        );
        assert!(
            get_type(&string::utf8(b"\".234\"")) == JSON_VALUE_TYPE_STRING,
            4
        );
        assert!(
            get_type(&string::utf8(b"[1234]")) == JSON_VALUE_TYPE_ARRAY,
            5
        );
    }
    ...
    fun test_string_to_stringify_number() {
        ...
        let test_str = string::utf8(
            b"-123999932124123.12056078123098123"
        );
        let res = parse_number(test_str);
        assert!(res.type == NUMBER_TYPE_DEC, 8);
        assert!(
            res.value == 123999932124123120560781230981230,
            9
        );
    ...
    fun test_string_to_number_exceeding_max_u64() {
        ...
        assert!(res.type == NUMBER_TYPE_INT, 0);
        assert!(
            res.value == 18_446_744_073_709_551_615,
            1
        );
        assert!(res.is_positive == true, 2);
        let res_string = stringify_number(res);
        ...
        assert!(res.type == NUMBER_TYPE_INT, 4);
        assert!(
            res.value == 18_446_744_073_709_551_616,
            5
        );
    ...
    fun test_string_to_number_max_u256() {
        ...
        assert!(res.type == NUMBER_TYPE_INT, 0);
        assert!(
            res.value == 115792089237316195423570985008687907853269984665640564039457584007913129639935
                ,
            1
        );
    ...
    fun test_parse_array() {
        ...
        assert!(
            value.value == string::utf8(b"{\"1\":\"2\"}"),
            8
        );
        let value = vector::borrow(&res, 1);
        assert!(
            value.value == string::utf8(b"{\"3\":\"4\"}"),
            9
        );
        let value = vector::borrow(&res, 2);
        assert!(
            value.value == string::utf8(b"{\"5\":\"6\"}"),
            10
        );
    }
    ...
    fun test_parse_object() {
        ...
        assert!(
            res_abc.type == JSON_VALUE_TYPE_NUMBER,
            0
        );
        assert!(
            res_abc.key == string::utf8(b"abc"),
            1
        );
        assert!(
            res_abc.value == string::utf8(b"18446744073709551615"),
            2
        );

        let res_def = vector::borrow(&res, 1);
        assert!(
            res_def.type == JSON_VALUE_TYPE_NUMBER,
            3
        );
        assert!(
            res_def.key == string::utf8(b"def"),
            4
        );
        assert!(
            res_def.value == string::utf8(b"18446744073709551616"),
            5
        );
    }
    ...
    fun test_parse_object2() {
        ...
        assert!(
            elem0.key == option::none<String>(),
            0
        );
        assert!(
            elem0.value.type == JSON_VALUE_TYPE_OBJECT,
            1
        );
        ...
        assert!(
            elem00.key == option::some<String>(string::utf8(b"abc")),
            2
        );
        assert!(
            elem00.value.type == JSON_VALUE_TYPE_NUMBER,
            3
        );
        ...
        assert!(
            elem00.value.value_number == option::some<Number>(expected_value00),
            4
        );
        ...
        assert!(
            elem01.key == option::some<String>(string::utf8(b"def")),
            7
        );
        assert!(
            elem01.value.type == JSON_VALUE_TYPE_OBJECT,
            8
        );
        ...
        assert!(
            elem010.key == option::some<String>(string::utf8(b"d")),
            9
        );
        assert!(
            elem010.value.type == JSON_VALUE_TYPE_ARRAY,
            10
        );
        ...
        assert!(
            elem0100.key == option::none<String>(),
            11
        );
        assert!(
            elem0100.value.type == JSON_VALUE_TYPE_NUMBER,
            12
        );
        ...
        assert!(
            elem0100.value.value_number == option::some<Number>(expected_value0100),
            13
        );
        ...
        assert!(
            elem0101.key == option::none<String>(),
            14
        );
        assert!(
            elem0101.value.type == JSON_VALUE_TYPE_NUMBER,
            15
        );
        ...
        assert!(
            elem0101.value.value_number == option::some<Number>(expected_value0101),
            16
        );
        ...
        assert!(
            elem0102.key == option::none<String>(),
            17
        );
        assert!(
            elem0102.value.type == JSON_VALUE_TYPE_NUMBER,
            18
        );
        ...
        assert!(
            elem0102.value.value_number == option::some<Number>(expected_value0102),
            19
        );
    }
    ...
    fun test_stringify_array() {
        ...
        set_array(
            &mut obj,
            get_next_index(&index, 3),
            option::none<String>(),
            0
        );
        set_object(
            &mut obj,
            get_next_index(&index, 4),
            option::none<String>(),
            0
        );
