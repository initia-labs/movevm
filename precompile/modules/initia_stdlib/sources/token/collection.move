...
module initia_std::collection {
    ...
    public fun nfts<T: key>(
        ...
        while (
            table::prepare<String, address>(nfts_iter) && vector::length(&res) <(limit as u64)
        ) {
    ...
    fun test_nfts_query(creator: &signer) acquires Collection, FixedSupply, UnlimitedSupply {
        ...
        assert!(
            nfts == vector[
                NftResponse {
                    token_id: string::utf8(b"3"),
                    nft: @0x003
                },
                NftResponse {
                    token_id: string::utf8(b"2"),
                    nft: @0x002
                },
                NftResponse {
                    token_id: string::utf8(b"1"),
                    nft: @0x001
                },
            ],
            0
