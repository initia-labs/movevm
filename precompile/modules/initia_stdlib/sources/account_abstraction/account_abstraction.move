module initia_std::account_abstraction {
    use std::bcs;
    use std::hash;
    ...
    use initia_std::ordered_map::{Self, OrderedMap};
    use initia_std::big_ordered_map::{Self, BigOrderedMap};
    use initia_std::account;
    use initia_std::event;
    use initia_std::function_info::{Self, FunctionInfo};
    ...
    enum DispatchableAuthenticator has key, copy, drop {
        V1 {
            auth_functions: OrderedMap<FunctionInfo, bool>
        }
    }
    ...
    enum DerivableDispatchableAuthenticator has key {
        V1 {
            auth_functions: BigOrderedMap<FunctionInfo, DerivableRegisterValue>
        }
    }
    ...
    public fun dispatchable_authenticator(
        addr: address
    ): Option<vector<FunctionInfo>> acquires DispatchableAuthenticator {
    ...
    public fun derive_account_address(
        derivable_func_info: FunctionInfo, abstract_public_key: &vector<u8>
    ): address {
    ...
    entry fun add_authentication_function(
        ...
        assert!(
            !is_permissioned_signer(account),
    ...
    entry fun remove_authenticator(account: &signer) acquires DispatchableAuthenticator {
        ...
        if (exists<DispatchableAuthenticator>(resource_addr)) {
            ...
            event::emit(RemoveDispatchableAuthenticator { account: addr });
        };
    }
    ...
    public entry fun register_derivable_authentication_function(
        ...
        DerivableDispatchableAuthenticator[@initia_std].auth_functions.add(
            function_info::new_function_info_from_address(
                module_address, module_name, function_name
            ),
    ...
    inline fun resource_addr(source: address): address {
        object::create_user_derived_object_address(source, @initia_std)
    }

    fun update_dispatchable_authenticator_impl(
        account: &signer, auth_function: FunctionInfo, is_add: bool
    ) acquires DispatchableAuthenticator {
        let addr = signer::address_of(account);
        let resource_addr = resource_addr(addr);
        let dispatcher_auth_function_info =
            function_info::new_function_info_from_address(
                @initia_std,
        ...
        assert!(
            function_info::check_dispatch_type_compatibility(
                &dispatcher_auth_function_info, &auth_function
            ),
        ...
        if (is_add) {
            if (!exists<DispatchableAuthenticator>(resource_addr)) {
                move_to(
                    &account::create_signer(resource_addr),
                    DispatchableAuthenticator::V1 { auth_functions: ordered_map::new() }
                );
            };
            let current_map =
                &mut borrow_global_mut<DispatchableAuthenticator>(resource_addr).auth_functions;
            assert!(
            ...
            assert!(
                exists<DispatchableAuthenticator>(resource_addr),
    ...
    inline fun dispatchable_derivable_authenticator_internal()
        ...
        assert!(
            exists<DerivableDispatchableAuthenticator>(@initia_std),
            error::not_found(EDERIVABLE_AA_NOT_INITIALIZED)
        );
        &DerivableDispatchableAuthenticator[@initia_std].auth_functions
    }

    fun authenticate(
        account: signer, func_info: FunctionInfo, signing_data: AbstractionAuthData
        ...
        if (signing_data.is_derivable()) {
            assert!(
                master_signer_addr
                    == derive_account_address(
                        func_info, signing_data.derivable_abstract_public_key()
                    ),
