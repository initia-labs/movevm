module initia_std::decimal128 {
    ...
    public fun from_string(num: &String): Decimal128 {
        ...
        let frac_part_val = if (dot_index == len) { 0 } else {
        ...
        new(
            int_part * DECIMAL_FRACTIONAL + frac_part_val
        )
    }
    ...
    fun test() {
        ...
        assert!(
            from_string(
                &string::utf8(b"340282366920938463462")
            ) == new(
        ...
        assert!(
            from_string(
                &string::utf8(b"340282366920938463462.0")
            ) == new(
                340282366920938463462 * DECIMAL_FRACTIONAL
            ),
            0
    ...
    fun test_round_up() {
        ...
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.0"))
            ) == 200,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.1"))
            ) == 200,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.2"))
            ) == 200,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.3"))
            ) == 200,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.4"))
            ) == 200,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.5"))
            ) == 201,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.6"))
            ) == 201,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.7"))
            ) == 201,
            0
        );
        assert!(
            round_up_u64(
                &from_string(&string::utf8(b"200.8"))
            ) == 201,
            0
        );
    ...
    fun failed_out_of_range() {
        _ = from_string(
            &string::utf8(b"340282366920938463463.0")
        );
    }
}
