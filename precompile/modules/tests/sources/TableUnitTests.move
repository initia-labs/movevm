module TestAccount::TableUnitTests {
    ...
    fun test_vector(s: signer) acquires S {
        ...
        assert!(
            vector::length(T::borrow(&s.t, 42)) == 2,
            105
        );
        assert!(
    ...
    fun test_table_of_tables(s: signer) {
        ...
        assert!(
            T::contains(T::borrow(&t, @0x12), @0xAB),
            101
        );
        assert!(
    ...
    #[test(initia_std = @initia_std, s = @0x42)]
    #[expected_failure(abort_code = 0x10064, location = initia_std::table)]
    ...
    #[test(initia_std = @initia_std, s = @0x42)]
    #[expected_failure(abort_code = 0x10065, location = initia_std::table)]
    ...
    fun test_iterator() {
        ...
        assert!(T::prepare<u64, u64>(iter), 103);
        let (key, value) = T::next<u64, u64>(iter);
        assert!(key == 3, 103);
        assert!(value == &3, 103);

        assert!(T::prepare<u64, u64>(iter), 104);
        let (key, value) = T::next<u64, u64>(iter);
        assert!(key == 4, 104);
        assert!(value == &4, 104);

        assert!(!T::prepare<u64, u64>(iter), 105);
